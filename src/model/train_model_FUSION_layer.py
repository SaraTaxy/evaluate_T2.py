...
for fold in fold_list:
    for elem in cfg['model']:
        #break
        # Dir
        model_dir = os.path.join(model_dir, elem)
        model_fold_dir = os.path.join(model_dir, str(fold))
        util_general.create_dir(model_fold_dir)
        report_dir = os.path.join(report_dir, elem)
        ...
        model = util_model.initialize_joint_model(fold, model_name=model_name, num_classes=len(classes), cfg_model=cfg['model'][elem], device=device)
